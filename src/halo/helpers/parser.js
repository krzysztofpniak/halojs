...
const Parser = daggy.tagged('Parser', ['fn']);
...
const RunParser = daggy.taggedSum('RunParser', {
  More: ['step'],
  Lift: ['m'],
  Stop: ['state', 'result'],
});
...
const Step = daggy.taggedSum('Step', {Loop: ['a'], Done: ['b']});

const ParseError = daggy.tagged('ParseError', ['message', 'position']);

const ParseState = daggy.tagged('ParseState', [
  'input',
  'position',
  'consumed',
]);

const Position = daggy.tagged('Position', ['index', 'line', 'column']);
